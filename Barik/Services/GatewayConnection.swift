// ABOUTME: WebSocket client for the Clawdis gateway using URLSessionWebSocketTask.
...
class GatewayConnection: NSObject, ObservableObject, URLSessionWebSocketDelegate {
    ...
    private let config: GhostyConfig.GatewayConfig
    ...
    init(port: Int, config: GhostyConfig.GatewayConfig) {
        self.port = port
        self.config = config
        super.init()
    ...
    private func sendConnectFrame(nonce: String?) {
        ...
        let authToken = storedDeviceToken ?? config.password
        ...
        if let password = config.password {
            authInfo = ConnectParams.AuthInfo(password: password, token: authToken)
        ...
        send(method: "connect", params: params) { [weak self] result in
            ...
            case .success(let response):
                if response.ok {
                    ...
                    self?.startHealthCheck(interval: self?.config.healthCheckInterval ?? 15)
                } else {
    ...
    private func scheduleReconnect() {
        guard shouldReconnect, !isReconnecting, reconnectAttempts < config.reconnectMaxAttempts else { return }
        isReconnecting = true
        reconnectAttempts += 1
        let delay = min(
            config.reconnectBaseDelay * pow(2.0, Double(reconnectAttempts - 1)),
