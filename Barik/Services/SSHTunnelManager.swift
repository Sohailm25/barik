...
class SSHTunnelManager: ObservableObject {
    ...
    private let config: GhostyConfig.TunnelConfig
    private let gatewayConfig: GhostyConfig.GatewayConfig
    private var shouldReconnect = true

    init(config: GhostyConfig.TunnelConfig, gatewayConfig: GhostyConfig.GatewayConfig) {
        self.config = config
    ...
    func startTunnel() {
        ...
        checkTCPConnection(host: "127.0.0.1", port: config.localPort) { [weak self] reachable in
    ...
    private func startHealthCheck() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            self.healthCheckTimer = Timer.scheduledTimer(
                withTimeInterval: self.gatewayConfig.healthCheckInterval,
                repeats: true
            ) { [weak self] _ in
    ...
    private func scheduleReconnect() {
        ...
        guard reconnectAttempts < gatewayConfig.reconnectMaxAttempts else {
            DispatchQueue.main.async { [weak self] in
        ...
        reconnectAttempts += 1
        let delay = min(
            gatewayConfig.reconnectBaseDelay * pow(2.0, Double(reconnectAttempts - 1)),
            gatewayConfig.reconnectMaxDelay
        )
