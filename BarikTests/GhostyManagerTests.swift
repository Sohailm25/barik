// ABOUTME: Tests for GhostyManager orchestrator covering message lifecycle and connection state.
// ABOUTME: Validates send flow, streaming events, message queueing, and error handling.

import XCTest
@testable import Barik

final class GhostyManagerTests: XCTestCase {

    private func makeManager() -> GhostyManager {
        return GhostyManager(gateway: nil)
    }
    ...
    func testIsStreamingDuringActiveRun() {
        let manager = makeManager()
        XCTAssertFalse(manager.session.isStreaming)

        manager.session.activeRunId = "run-42"
        XCTAssertTrue(manager.session.isStreaming)
    }
