...
final class GatewayConnectionTests: XCTestCase {
    ...
    func testConnectParamsEncodesProtocolVersion() throws {
        ...
        XCTAssertEqual(client["id"] as? String, "clawdbot-macos")
        XCTAssertEqual(client["displayName"] as? String, "Barik")
    ...
    func testDeviceIdentityGeneratesValidId() {
        let identity = DeviceIdentity()
        XCTAssertEqual(identity.deviceId.count, 64)
        XCTAssertFalse(identity.publicKeyRawBase64Url.isEmpty)
    }

    func testDeviceIdentitySignsPayload() {
        let identity = DeviceIdentity()
        let signature = identity.sign(payload: "test-payload")
    ...
    func testIsConnectedDefaultsFalse() {
        let config = GhostyConfig.GatewayConfig()
        let connection = GatewayConnection(port: 99999, config: config)
        XCTAssertFalse(connection.isConnected)
    }

    func testHealthCheckIntervalFromConfig() {
        let config = GhostyConfig.GatewayConfig()
        XCTAssertEqual(config.healthCheckInterval, 15)
    }

    func testReconnectMaxAttemptsFromConfig() {
        let config = GhostyConfig.GatewayConfig()
        XCTAssertEqual(config.reconnectMaxAttempts, 10)
    }

    func testReconnectBaseDelayFromConfig() {
        let config = GhostyConfig.GatewayConfig()
        XCTAssertEqual(config.reconnectBaseDelay, 1.0)
    }

    func testReconnectMaxDelayFromConfig() {
        let config = GhostyConfig.GatewayConfig()
        XCTAssertEqual(config.reconnectMaxDelay, 30.0)
    }

    func testPasswordDefaultsToNil() {
        let config = GhostyConfig.GatewayConfig()
